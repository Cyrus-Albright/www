...
from werkzeug import secure_filename  # 不行就从werkzeug.utils导入
import requests
import os
import json
...
def downloadfile():
    ...
    # DONE：安全问题：验证文件路径，防入侵！删除视图函数同理！
    # 原方案：filename = request.args.get("filename")

    # 获取文件完整路径
    absfilepath = os.path.join(os.path.abspath(FILECACHEDIR), filename)
    print(
        f"[Route:downloadfile()]dir:{FILECACHEDIR}, filename:{filename}, abspath:{absfilepath}")
    # 判断文件是否存在
    if not (os.path.exists(absfilepath) or os.path.isfile(absfilepath)):
